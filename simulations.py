...
def get_placement_algorithm(algorithm: float) -> CirclePlacerFunction:
    ...
    if algorithm == 4:
        ...
        return lambda p, pk: place_algorithm_11(p, pk, 14, initial_guess=initial_guess_find_one if pk == default_pk else initial_guess_find_all)
    else:
...
def run_search(evaluator: Callable[[float], tuple[bool, list[Circle]]], debug: bool = False) -> tuple[float, list[Circle]]:
    ...
    # return binary_search(0.76, 0.763, evaluator, debug=debug)
